...
#include "include/lib/registerer/registerer.h"
#include <iostream>
namespace apollo {
...
bool GetRegisteredClasses(
  ...
  if (registered_derived_classes_names == nullptr) {
    std::cout << "AERROR  ------ " << "registered_derived_classes_names is not available";
    return false;
  }
  BaseClassMap &map = GlobalFactoryMap();
  auto iter = map.find(base_class_name);
  if (iter == map.end()) {
    std::cout << "AERROR  ------ " << "class not registered:" << base_class_name;
    return false;
  }
