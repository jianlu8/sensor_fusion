 ...
 *****************************************************************************/
#include "include/lib/fusion_system/probabilistic_fusion/probabilistic_fusion.h"

#include <map>
#include <utility>
#include <iostream>
#include "include/util/file.h"
#include "include/util/perf_util.h"
#include "include/util/string_util.h"
#include "include/util/object_pool_types.h"
#include "include/base/base_init_options.h"
#include "include/base/track_pool_types.h"
#include "include/lib/data_association/hm_data_association/hm_tracks_objects_match.h"
#include "include/lib/data_fusion/dst_existence_fusion.h"
#include "include/lib/data_fusion/pbf_tracker.h"
#include "include/lib/data_fusion/dst_type_fusion.h"
#include "include/lib/gatekeeper/pbf_gatekeeper/pbf_gatekeeper.h"
#include "include/lib/config_manager/config_manager.h"
...
bool ProbabilisticFusion::Init(const FusionInitOptions& init_options) {
  ...
  std::string work_root_config = "/work_root_config";

  std::string config = "/config/probabilistic_fusion_config.pb.txt";
  ProbabilisticFusionConfig params;

  if (!cyber::common::GetProtoFromFile(config, &params)) {
    std::cout << "AERROR  ------ " << "Read config failed: " << config;
    return false;
  }
  ...
  if (params_.data_association_method == "HMAssociation") {
    ...
    std::cout << "AERROR  ------ " << "Unknown association method: " << params_.data_association_method;
    return false;
  }
  if (!matcher_->Init()) {
    std::cout << "AERROR  ------ " << "Failed to init matcher.";
    return false;
  }
  ...
  if (params_.gate_keeper_method == "PbfGatekeeper") {
    ...
    std::cout << "AERROR  ------ " << "Unknown gate keeper method: " << params_.gate_keeper_method;
    return false;
  }
  if (!gate_keeper_->Init()) {
    std::cout << "AERROR  ------ " << "Failed to init gatekeeper.";
    return false;
  }
...
bool ProbabilisticFusion::Fuse(const FusionOptions& options,
  ...
  if (fused_objects == nullptr) {
    std::cout << "AERROR  ------ " << "fusion error: fused_objects is nullptr";
    return false;
  }
  ...
  std::cout << "Get " << frames.size() << " related frames for fusion";
...
void ProbabilisticFusion::RemoveLostTrack() {
  ...
  std::cout << "Remove " << foreground_tracks.size() - foreground_track_count
  ...
  std::cout << "Remove " << background_tracks.size() - background_track_count
        << " background tracks";
...
void ProbabilisticFusion::CollectObjectsByTrack(
  ...
  *obj = *(fused_object->GetBaseObject());
  ...
  // std::cout << "ADEBUG " << "fusion_reporting..." << obj->track_id << "@"
