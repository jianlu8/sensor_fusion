...
#include <iostream>
#include "include/common/environment.h"
#include "include/util/file.h"
#include "include/util/log.h"
// #include "include/common/io/io_util.h"
#include "include/common/perception_gflags.h"
...
bool ConfigManager::InitInternal() {
  ...
  for (const auto &model_config_file : model_config_files) {
    ...
    if (!GetProtoFromASCIIFile(model_config_file, &file_list_proto)) {
      std::cout << "AERROR  ------ " << "Invalid ModelConfigFileListProto file: " << model_config_file;
      return false;
    }
    ...
    for (const std::string &model_config_path :
      ...
      if (!GetProtoFromASCIIFile(abs_path, &multi_model_config_proto)) {
        std::cout << "AERROR  ------ " << "Invalid MultiModelConfigProto file: " << abs_path;
        return false;
      }
      ...
      for (const ModelConfigProto &model_config_proto :
        ...
        if (!result.second) {
          std::cout << " WARN    " << "duplicate ModelConfig, name: " << model_config->name();
          return false;
        }
      }
    }
