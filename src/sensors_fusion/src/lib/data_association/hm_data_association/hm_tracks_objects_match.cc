...
#include "include/common/secure_matrix.h"
...
bool HMTrackersObjectsAssociation::Associate(
  ...
  std::cout << "ADEBUG " << "association_measurement_timestamp@" << measurement_timestamp;
  ...
  std::cout << "association: measurement_num = " << sensor_objects.size()
  ...
  for (size_t i = 0; i < unassigned_tracks.size(); ++i) {
    ...
    for (size_t j = 0; j < unassigned_measurements.size(); ++j) {
      ...
      if (center_dist < s_association_center_dist_threshold_) {
        ...
        std::cout << "ADEBUG " << "center_distance " << center_dist
      ...
      std::cout << "ADEBUG " << "track_id: " << fusion_track->GetTrackId()
             << ", obs_id: " << sensor_object->GetBaseObject()->track_id
             << ", distance: " << distance;
    }
  }
