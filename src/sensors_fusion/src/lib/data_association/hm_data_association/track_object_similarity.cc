...
double ComputePtsBoxLocationSimilarity(const ProjectionCachePtr& cache,
  ...
  if (object->Empty()) {
    std::cout << "ADEBUG " << "cache object is empty!";
    return min_p;
  }
  ...
  if (end_ind - start_ind >= check_augmented_iou_minimum_pts_num) {
    ...
    if (augmented_iou < kFloatEpsilon) {
      std::cout << "ADEBUG " << "augmented iou is empty!";
      return min_p;
    }
  }
  for (size_t i = start_ind; i < end_ind; ++i) {
    ...
    if (velo_pt2d == nullptr) {
      std::cout << "AERROR  ------ " << "query pt from projection cache failed!";
      continue;
    }
  ...
  std::cout << "ADEBUG " << "mean_pixel_dist is: " << mean_pixel_dist;
...
double ComputePtsBoxShapeSimilarity(const ProjectionCachePtr& cache,
  ...
  std::cout << "ADEBUG " << "camera_box_size@(" << camera_box_size(0) << ","
         << camera_box_size(1) << "); "
...
double ComputePtsBoxSimilarity(const ProjectionCachePtr& cache,
  ...
  // std::cout << "ADEBUG " << "fused_similarity@" << fused_similarity << ", location_similarity@"
