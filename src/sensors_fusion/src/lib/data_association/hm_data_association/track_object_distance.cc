 ...
 *****************************************************************************/
#include "include/lib/data_association/hm_data_association/track_object_distance.h"
...
#include "include/util/sensor_meta.h"
// #include "include/common/camera_homography.h"
#include "include/lib/data_association/hm_data_association/chi_squared_cdf_1_0.0500_0.999900.h"
#include "include/lib/data_association/hm_data_association/chi_squared_cdf_2_0.0500_0.999900.h"
...
float TrackObjectDistance::Compute(const TrackPtr& fused_track,
  ...
  if (fused_object == nullptr) {
    std::cout << "AERROR  ------ " << "fused object is nullptr";
    return (std::numeric_limits<float>::max)();
  }
  Eigen::Vector3d* ref_point = options.ref_point;
  if (ref_point == nullptr) {
    std::cout << "AERROR  ------ " << "reference point is nullptr";
    return (std::numeric_limits<float>::max)();
  }
  ...
  if (IsLidar(sensor_object)) {
    ...
    std::cout << "AERROR  ------ " << "fused sensor type is not support";
  }
  return min_distance;
}
...
float TrackObjectDistance::ComputeLidarLidar(
  ...
  if (center_distance > s_lidar2lidar_association_center_dist_threshold_) {
    std::cout << "ADEBUG " << "center distance exceed lidar2lidar tight threshold: "
  ...
  std::cout << "ADEBUG " << "ComputeLidarLidar distance: " << distance;
  return distance;
}
...
float TrackObjectDistance::ComputeLidarRadar(
  ...
  if (center_distance > s_lidar2radar_association_center_dist_threshold_) {
    std::cout << "ADEBUG " << "center distance exceed lidar2radar tight threshold: "
           << "center_dist@" << center_distance << ", "
           << "tight_threh@"
  ...
  float distance =
      ComputePolygonDistance3d(fused_object, sensor_object, ref_pos, range);
  std::cout << "ADEBUG " << "ComputeLidarRadar distance: " << distance;
  return distance;
}
...
float TrackObjectDistance::ComputeRadarRadar(
  ...
  if (center_distance > s_radar2radar_association_center_dist_threshold_) {
    std::cout << "ADEBUG " << "center distance exceed radar2radar tight threshold: "
           << "center_dist@" << center_distance << ", "
           << "tight_threh@"
  ...
  float distance =
      ComputePolygonDistance3d(fused_object, sensor_object, ref_pos, range);
  std::cout << "ADEBUG " << "ComputeRadarRadar distance: " << distance;
  return distance;
}
...
float TrackObjectDistance::ComputeLidarCamera(
  ...
  // std::cout << "ADEBUG " << "object cloud size : " << cloud.size();
  // if (cloud.size() > 0) {
  ...
  //   }
  // }
  // std::cout << "ADEBUG " << "ComputeLidarCamera distance: " << distance;
  // return distance;
}
...
float TrackObjectDistance::ComputeRadarCamera(
  ...
  // std::cout << "ADEBUG " << "ComputeRadarCamera distance: " << distance;
  // return distance;
}
