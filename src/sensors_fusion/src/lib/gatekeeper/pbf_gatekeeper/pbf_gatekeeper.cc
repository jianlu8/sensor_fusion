...
#include "include/util/object_types.h"
#include "include/base/base_init_options.h"
#include "include/lib/gatekeeper/pbf_gatekeeper/proto/pbf_gatekeeper_config.pb.h"
...
bool PbfGatekeeper::RadarAbleToPublish(const TrackPtr &track, bool is_night) {
  ...
  if (params_.publish_if_has_radar && visible_in_radar &&
    ...
    if (radar_object->GetSensorId() == "radar_front") {
      ...
      std::cout << "ADEBUG " << "radar_rear: min_dis: " << params_.min_radar_confident_distance
             << " obj dist: "
  ...
  if (params_.publish_if_has_camera && visible_in_camera &&
    ...
    if ((camera_object->GetBaseObject()->sub_type ==
      ...
      if (exist_prob > params_.existence_threshold) {
        ...
        std::cout << "publish camera only object : cnt =  " << cnt_cam;
        cnt_cam++;
        return true;
      }
    }
