...
#include "include/common/basic.h"
#include "include/common/sensor_manager/sensor_manager.h"
...
void KalmanMotionFusion::MotionFusionWithMeasurement(
  ...
  std::cout << "ADEBUG " << "fusion_original_measurement@(" << std::setprecision(10)
         << observation(0) << "," << observation(1) << "," << observation(2)
         << "," << observation(3) << ")";
  std::cout << "ADEBUG " << "fusion_original_measurement_covariance@(" << r_matrix(0, 0) << ","
  ...
  std::cout << "ADEBUG " << "fusion_pseudo_measurement@(" << std::setprecision(10)
         << observation(0) << "," << observation(1) << "," << observation(2)
         << "," << observation(3) << ")";
  std::cout << "ADEBUG " << "fusion_pseudo_measurement_covariance@(" << r_matrix(0, 0) << ","
         << r_matrix(0, 1) << "," << r_matrix(1, 0) << "," << r_matrix(1, 1)
         << "," << r_matrix(2, 2) << "," << r_matrix(2, 3) << ","
         << r_matrix(3, 2) << "," << r_matrix(3, 3) << ")";
...
Eigen::VectorXd KalmanMotionFusion::ComputeAccelerationMeasurement(
  ...
  if (GetSensorHistoryLength(sensor_type) >= s_eval_window_) {
    ...
    if (history_index >= history_velocity_.size()) {
      std::cout << "AERROR  ------ " << "illegal history index";
...
Eigen::Vector4d KalmanMotionFusion::ComputePseudoMeasurement(
  ...
  std::cout << "unsupport sensor type for pseudo measurement computation!";
