...
#include "include/base/sensor_data_manager.h"
// #include "include/common/camera_util.h"
...
bool DstExistenceFusion::Init() {
  ...
  if (!GetFusionInitOptions("DstExistenceFusion", &options)) {
    std::cout << "AERROR  ------ " << "GetFusionInitOptions failed ";
    return false;
  }
  ...
  for (auto valid_dist : params.camera_valid_dist()) {
    ...
    std::cout << "dst existence fusion params: " << camera_id
  ...
  std::cout << "dst existence fusion params: "
...
void DstExistenceFusion::UpdateWithMeasurement(
  ...
  std::cout << "ADEBUG " << " before update exist prob: " << GetExistenceProbability();
  fused_existence_ =
      fused_existence_ + existence_evidence * exist_fused_w * association_prob;
  std::cout << "ADEBUG " << " update with, EXIST prob: " << GetExistenceProbability()
...
void DstExistenceFusion::UpdateWithoutMeasurement(const std::string &sensor_id,
  ...
  if ((lidar_object != nullptr && lidar_object->GetSensorId() == sensor_id) ||
    ...
    std::cout << "ADEBUG " << " update without, EXIST prob: " << GetExistenceProbability()
...
double DstExistenceFusion::ComputeFeatureInfluence(
  ...
  std::cout << "ADEBUG " << " sensor_id: " << measurement->GetSensorId()
...
void DstExistenceFusion::UpdateToicWithCameraMeasurement(
  ...
  //                sensor_id;
  // } else {
  //   std::cout << "ADEBUG " << "camera dist: " << sensor_id << " " << max_dist_it->second;
  // }
  // if (status && max_dist_it != options_.camera_max_valid_dist_.end()) {
  //   SensorObjectConstPtr lidar_object = track_ref_->GetLatestLidarObject();
  //   SensorObjectConstPtr radar_object = track_ref_->GetLatestRadarObject();
...
void DstExistenceFusion::UpdateExistenceState() {
  ...
  if (!(track_ref_->GetLidarObjects()).empty()) {
    ...
    std::cout << "AERROR  ------ " << boost::format("this fused object: %d has no sensor objects") %
