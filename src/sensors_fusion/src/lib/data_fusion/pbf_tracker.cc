...
#include "include/lib/data_fusion/kalman_motion_fusion.h"

#include "include/lib/data_fusion/pbf_shape_fusion.h"
...
bool PbfTracker::InitParams() {
  ...
  std::cout << "Load PbfTrackerConfig: " << params.type_fusion_method() << ","
...
bool PbfTracker::InitMethods() {
  if (s_type_fusion_method_ == "DstTypeFusion") {
    ...
    std::cout << "AERROR  ------ " << "Unknown type fusion : " << s_type_fusion_method_;
    return false;
  }
  ...
  if (s_motion_fusion_method_ == "KalmanMotionFusion") {
    ...
    std::cout << "AERROR  ------ " << "Unknown motion fusion : " << s_motion_fusion_method_;
    return false;
  }
  ...
  if (s_existence_fusion_method_ == "DstExistenceFusion") {
    ...
    std::cout << "AERROR  ------ " << "Unknown existence fusion : " << s_existence_fusion_method_;
    return false;
  }
  ...
  if (s_shape_fusion_method_ == "PbfShapeFusion") {
    ...
    std::cout << "AERROR  ------ " << "Unknown shape fusion : " << s_shape_fusion_method_;
    return false;
  }
...
void PbfTracker::UpdateWithMeasurement(const TrackerOptions& options,
  ...
  // std::cout << "ADEBUG " << "fusion_updating..." << track_->GetTrackId() << " with "
