...
bool SetProtoToASCIIFile(const google::protobuf::Message &message,
  ...
  if (file_descriptor < 0) {
    std::cout << "AERROR  ------ " << "Invalid file descriptor.";
    return false;
  }
...
bool SetProtoToASCIIFile(const google::protobuf::Message &message,
  ...
  if (fd < 0) {
    std::cout << "AERROR  ------ " << "Unable to open file " << file_name << " to write.";
    return false;
  }
...
bool GetProtoFromASCIIFile(const std::string &file_name,
  ...
  if (file_descriptor < 0) {
    std::cout << "AERROR  ------ " << "Failed to open file " << file_name << " in text mode.";
    // Failed to open;
    return false;
  }
  ...
  if (!success) {
    std::cout << "AERROR  ------ " << "Failed to parse file " << file_name << " as text proto.";
  }
  delete input;
  close(file_descriptor);
  return success;
...
bool GetProtoFromBinaryFile(const std::string &file_name,
  ...
  if (!input.good()) {
    std::cout << "AERROR  ------ " << "Failed to open file " << file_name << " in binary mode.";
    return false;
  }
  if (!message->ParseFromIstream(&input)) {
    std::cout << "AERROR  ------ " << "Failed to parse file " << file_name << " as binary proto.";
    return false;
  }
  return true;
}
...
bool CopyFile(const std::string &from, const std::string &to) {
  ...
  if (!src) {
    std::cout << " WARN    " << "Source path could not be normally opened: " << from;
    std::string command = "cp -r " + from + " " + to;
    std::cout << "ADEBUG " << command;
    const int ret = std::system(command.c_str());
    if (ret == 0) {
      std::cout << "ADEBUG " << "Copy success, command returns " << ret;
      return true;
    } else {
      std::cout << "ADEBUG " << "Copy error, command returns " << ret;
      return false;
    }
  }
  ...
  if (!dst) {
    std::cout << "AERROR  ------ " << "Target path is not writable: " << to;
    return false;
  }
...
bool CopyDir(const std::string &from, const std::string &to) {
  ...
  if (directory == nullptr) {
    std::cout << "AERROR  ------ " << "Cannot open directory " << from;
    return false;
  }
  ...
  if (EnsureDirectory(to)) {
    ...
    std::cout << "AERROR  ------ " << "Cannot create target directory " << to;
    ret = false;
  }
  closedir(directory);
  return ret;
...
bool RemoveAllFiles(const std::string &directory_path) {
  ...
  if (directory == nullptr) {
    std::cout << "AERROR  ------ " << "Cannot open directory " << directory_path;
    return false;
  }
  ...
  while ((file = readdir(directory)) != nullptr) {
    ...
    if (unlink(file_path.c_str()) < 0) {
      std::cout << "AERROR  ------ " << "Fail to remove file " << file_path << ": " << strerror(errno);
      closedir(directory);
      return false;
    }
  }
...
bool GetType(const string &filename, FileType *type) {
  ...
  if (S_ISDIR(stat_buf.st_mode) != 0) {
    ...
    *type = TYPE_FILE;
    ...
    std::cout << " WARN    " << "failed to get type: " << filename;
    return false;
  }
  return true;
}
...
bool DeleteFile(const string &filename) {
  ...
  if (type == TYPE_FILE) {
    if (remove(filename.c_str()) != 0) {
      std::cout << "AERROR  ------ " << "failed to remove file: " << filename;
      return false;
    }
    return true;
  }
  ...
  if (dir == nullptr) {
    std::cout << " WARN    " << "failed to opendir: " << filename;
    return false;
  }
  ...
  while ((dir_info = readdir(dir)) != nullptr) {
    ...
    if (!GetType(temp_file, &temp_type)) {
      std::cout << " WARN    " << "failed to get file type: " << temp_file;
      closedir(dir);
      return false;
    }
    if (type == TYPE_DIR) {
...
bool CreateDir(const string &dir) {
  ...
  if (ret != 0) {
    std::cout << " WARN    " << "failed to create dir. [dir: " << dir
          << "] [err: " << strerror(errno) << "]";
    return false;
  }
  return true;
