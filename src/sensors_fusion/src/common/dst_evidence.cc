 ...
 *****************************************************************************/
#include "include/common/dst_evidence.h"
...
bool DstManager::AddApp(const std::string &app_name,
  ...
  if (dst_common_data_.find(app_name) != dst_common_data_.end()) {
    std::cout << " WARN    " << boost::format("Dst %s was added!") % app_name;
  }
...
DstCommonDataPtr DstManager::GetAppDataPtr(const std::string &app_name) {
  if (!IsAppAdded(app_name)) {
    std::cout << "AERROR  ------ " << "app_name is not available";
    return nullptr;
  }
  auto iter = dst_common_data_.find(app_name);
  if (iter != dst_common_data_.end()) {
...
bool Dst::SetBbaVec(const std::vector<double> &bba_vec) {
  ...
  if (bba_vec.size() != dst_data_ptr_->fod_subsets_.size()) {
    std::cout << "AERROR  ------ " << boost::format("input bba_vec size: %d !=  Dst subsets size: %d") %
  ...
  for (auto belief_mass : bba_vec) {
    if (belief_mass < 0.0) {
      std::cout << " WARN    " << boost::format(" belief mass: %lf is not valid") % belief_mass;
      return false;
    }
  }
  // reset
...
bool Dst::SetBba(const std::map<uint64_t, double> &bba_map) {
  ...
  for (auto bba_map_iter : bba_map) {
    ...
    if (find_res == subsets_ind_map.end()) {
      std::cout << "AERROR  ------ " << "the input bba map has invalid fod subset";
      return false;
    }
    if (belief_mass < 0.0) {
      std::cout << " WARN    " << boost::format("belief mass: %lf is not valid. Dst name: %s") %
...
void Dst::Normalize() {
  ...
  if (mass_sum == 0.0) {
    std::cout << "ADEBUG " << "mass_sum equal 0!!";
  }
