...
// #include "include/common/io/io_util.h"
...
bool SensorManager::Init() {
  ...
  if (!GetProtoFromASCIIFile(file_path, &sensor_list_proto)) {
    std::cout << "AERROR  ------ " << "Invalid MultiSensorMeta file: " << FLAGS_obs_sensor_meta_path;
    return false;
  }
  ...
  auto AddSensorInfo = [this](const SensorMeta& sensor_meta_proto) {
    ...
    if (!pair.second) {
      std::cout << "AERROR  ------ " << "Duplicate sensor name error.";
      return false;
    }
  ...
  for (const SensorMeta& sensor_meta_proto : sensor_list_proto.sensor_meta()) {
    if (!AddSensorInfo(sensor_meta_proto)) {
      std::cout << "AERROR  ------ " << "Failed to add sensor_info: " << sensor_meta_proto.name();
      return false;
    }
  }

  inited_ = true;
  std::cout << "Init sensor_manager success.";
  return true;
}
