...
#include "include/common/math/mpc_osqp.h"
#include <iostream>
...
MpcOsqp::MpcOsqp(const Eigen::MatrixXd &matrix_a,
  ...
  std::cout << "ADEBUG " << "state_dim" << state_dim_;
  std::cout << "ADEBUG " << "control_dim_" << control_dim_;
...
void MpcOsqp::CalculateGradient() {
  ...
  std::cout << "ADEBUG " << "Gradient_mat";
  std::cout << "ADEBUG " << gradient_;
}
...
void MpcOsqp::CalculateEqualityConstraint(std::vector<c_float> *A_data,
  ...
  std::cout << "ADEBUG " << "matrix_constraint with A";
  std::cout << "ADEBUG " << matrix_constraint;
  ...
  std::cout << "ADEBUG " << "matrix_constraint with B";
  std::cout << "ADEBUG " << matrix_constraint;
  ...
  std::cout << "ADEBUG " << "matrix_constraint with I";
  std::cout << "ADEBUG " << matrix_constraint;
...
void MpcOsqp::CalculateConstraintVectors() {
  ...
  std::cout << "ADEBUG " << " matrix_u_lower_";
  for (size_t i = 0; i < horizon_ + 1; i++) {
  ...
  std::cout << "ADEBUG " << " matrix_x_lower_";
  ...
  std::cout << "ADEBUG " << " matrix_initial_x_";
...
OSQPData *MpcOsqp::Data() {
  ...
  if (data == nullptr) {
    ...
    std::vector<c_int> P_indices;
    std::vector<c_int> P_indptr;
    std::cout << "ADEBUG " << "before CalculateKernel";
    CalculateKernel(&P_data, &P_indices, &P_indptr);
    std::cout << "ADEBUG " << "CalculateKernel done";
    data->P =
        csc_matrix(kernel_dim, kernel_dim, P_data.size(), CopyData(P_data),
                   CopyData(P_indices), CopyData(P_indptr));
    std::cout << "ADEBUG " << "Get P matrix";
    data->q = gradient_.data();
    std::cout << "ADEBUG " << "before CalculateEqualityConstraint";
    std::vector<c_float> A_data;
    std::vector<c_int> A_indices;
    std::vector<c_int> A_indptr;
    CalculateEqualityConstraint(&A_data, &A_indices, &A_indptr);
    std::cout << "ADEBUG " << "CalculateEqualityConstraint done";
    data->A =
...
bool MpcOsqp::Solve(std::vector<double> *control_cmd) {
  std::cout << "ADEBUG " << "Before Calc Gradient";
  CalculateGradient();
  std::cout << "ADEBUG " << "After Calc Gradient";
  CalculateConstraintVectors();
  std::cout << "ADEBUG " << "MPC2Matrix";

  OSQPData *data = Data();
  std::cout << "ADEBUG " << "OSQP data done";
  std::cout << "ADEBUG " << "OSQP data n" << data->n;
  std::cout << "ADEBUG " << "OSQP data m" << data->m;
  for (int i = 0; i < data->n; ++i) {
    std::cout << "ADEBUG " << "OSQP data q" << i << ":" << (data->q)[i];
  }
  std::cout << "ADEBUG " << "OSQP data l" << data->l;
  for (int i = 0; i < data->m; ++i) {
    std::cout << "ADEBUG " << "OSQP data l" << i << ":" << (data->l)[i];
  }
  std::cout << "ADEBUG " << "OSQP data u" << data->u;
  for (int i = 0; i < data->m; ++i) {
    std::cout << "ADEBUG " << "OSQP data u" << i << ":" << (data->u)[i];
  }

  OSQPSettings *settings = Settings();
  std::cout << "ADEBUG " << "OSQP setting done";
  OSQPWorkspace *osqp_workspace = nullptr;
  // osqp_setup(&osqp_workspace, data, settings);
  osqp_workspace = osqp_setup(data, settings);
  std::cout << "ADEBUG " << "OSQP workspace ready";
  osqp_solve(osqp_workspace);

  auto status = osqp_workspace->info->status_val;
  std::cout << "ADEBUG " << "status:" << status;
  // check status
  ...
  for (size_t i = 0; i < control_dim_; ++i) {
    ...
    std::cout << "ADEBUG " << "control_cmd:" << i << ":" << control_cmd->at(i);
  }
