...
void SolveLQRProblem(const Matrix &A, const Matrix &B, const Matrix &Q,
  ...
  if (A.rows() != A.cols() || B.rows() != A.rows() || Q.rows() != Q.cols() ||
    ...
    std::cout << "AERROR  ------ " << "LQR solver: one or more matrices have incompatible dimensions.";
    return;
  }
  ...
  if (num_iteration >= max_num_iteration) {
    std::cout << "ADEBUG " << "LQR solver cannot converge to a solution, "
              "last consecutive result diff is: "
           << diff;
  } else {
    std::cout << "ADEBUG " << "LQR solver converged at iteration: " << num_iteration
